<template>
    <div>
        ...
        <div class="relative">
            <img class="h-[60vh] w-full object-center object-cover object-no-repeat" :src="user?.cover_image" alt="">
        ...
        <div class="-translate-y-[40%] md:-translate-y-2/3 px-2 md:px-10">
            <div class="flex flex-col md:flex-row gap-y-4   justify-between items-center  md:text-white">
                <div>
                    <img class="h-40 w-40 border-2 border-white rounded-full" :src="user?.profile_image" alt="">
                ...
                <div class="flex gap-x-2">
                    <button @click="showCreateStory=true"
                        class="px-4 py-2 rounded-md border border-gray-300 cursor-pointer hover:border-transparent hover:bg-gray-200 transition-colors">
                        <span v-if="pending"><LoadingButton class="h-6 w-6" /></span>
                        <span v-elses>Create Story</span>
        ...
        <div class="px-2 md:px-10 -mt-32 md:-mt-28">
        ...
        <!-- create post -->
        <div v-if="showCreateStory">
            <CreatePost @close="showCreateStory = false; eStroy = {}; editMode = false" :Editstory="eStroy"
...
async function getStories() {
    ...
    try {
        const res = await $api(`user-stories?user_id=${user.value.user_id}&page=${page.value}`)
...
function handleEdit(id) {
    ...
    const estroy = stories.value.find(e => e.story_id === id)
    if (estroy) {
        eStroy.value = estroy
        editMode.value = true
    }
